...
three vertices are ordinary simple cycles.  This harmless degenerate case is
needed only inside the recursion.  The assembled root code below has at least
three vertices and therefore gives a simple Hamilton cycle.
...
Their internal edges are either edges of a cut child code or one of the two
...
chronology of its discovery.  The public bridge declaration
\leanname{NCRefinementGraph_adj_iff_single_merge} expands graph adjacency to
the explicit two-block merge in either direction; the rank argument in the
introduction identifies these steps with the order-theoretic covers.
Table~\ref{tab:lean-map} lists selected declarations corresponding to the
principal formal components of each paper statement.  It is a navigation
index, not a machine-checked equivalence between arbitrary prose and Lean.
The file
\texttt{paper/theorem-map.json} is the machine-readable source of this table,
and the package checker verifies that every paper label is present and that
every listed declaration exists, occurs in the table, and is kernel-checked
by the Lean theorem-map module.  The complete classification itself is
...
\texttt{lake env lean --trust=0 Hamilton/HamiltonianCyclesAxiomAudit.lean}
reports for
...
The machine audit establishes the stated Lean theorems under their formal
definitions and the foundational dependencies just listed.  The surrounding
natural-language identification of those definitions and hypotheses remains
part of the human mathematical exposition.
...
present manuscript revision adds expository proof detail and the public
single-merge bridge above without changing the classification theorem.
