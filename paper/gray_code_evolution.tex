...
Given a finite list $L$ of distinct elements of $\NC(n)$, a refinement Gray
code for $L$ is an ordering of all entries of $L$, each used once, such that
consecutive partitions are adjacent in $\NCR(n)$.  Merino, Namrata, and
Williams~\cite{MNW} prove that deciding whether an arbitrary input list $L$
admits such an ordering is NP-complete.  Their arbitrary-list problem differs
from the full-family input $L=\NC(n)$.  Corollary~\ref{cor:hamilton-paths}
shows that this full-family input admits a refinement Gray code if and only if
$n\leq3$ or $n$ is even.  For even $n\geq4$, the ordering may be chosen
cyclically, meaning that its last and first entries are also adjacent.  For
broader background on combinatorial Gray codes and their Hamiltonian
formulations, see M\"utze's survey~\cite{M}.
...
The Lean development proves the merge/split adjacency and the cycle and path
classifications for the full family $\NC(n)$.  It does not formalize the
arbitrary-list decision problem of Merino, Namrata, and Williams; the
comparison above follows directly from their definition.

\ifdefined\ssrnrevisiononly
\else
The theorem map and axiom audit are the case-specific mathematical evidence.
Their role within a claim-graph verification architecture is described
separately in \cite{LiProofEngine}.
\fi
...
harness components; the present work used OpenAI Codex and Claude Code within
that implementation.  The framework is described in \emph{Proof Engine
Infrastructure}~\cite{LiProofEngine}.  The author reviewed and edited the
...
\bibitem{LiProofEngine}
A.~C.~Li,
Proof Engine Infrastructure: A Claim-Graph Framework for AI-Assisted
Mathematical Research,
SSRN Scholarly Paper 7237460, \doi{10.2139/ssrn.7237460}, 2026.
