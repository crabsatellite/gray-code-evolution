...
the number of peaks.  Let $p(T)$ be this peak count.  A tree with $n+1$
nodes has the form $T=\operatorname{node}(L,R)$ with
...
chord, contradicting the root assertion.  Otherwise
...
every step, and has closing pair $\{u_{d-1}\},\varnothing$.  It also has the
following two least-significant-coordinate properties.  When $d\geq1$, for
every $H\subseteq F(Q)\setminus\{u_0\}$, the masks $H$ and
$H\cup\{u_0\}$ are consecutive.  When $d\geq2$, for every
$H\subseteq F(Q)\setminus\{u_0,u_1\}$, the masks
$H\cup\{u_0\}$ and $H\cup\{u_0,u_1\}$ are consecutive.  These assertions
follow together by induction from the same recursion.  Equation
...
\[
 ...
 M(\{0\})=\{0,i,j\}.
\]
By the first least-significant-coordinate property above, both the parent
pair and the child pair $\varnothing,\{0\}$ are edges of their fixed Gray
cycles.  The cross covers are
\[
...
\[
 ...
 M(\{0\})=\{0,1,j\}.
\]
By the second least-significant-coordinate property, the parent pair is a
Gray-cycle edge; by the first property, so is the child pair.  The cross
covers are
\[
...
The public script \texttt{scripts/verify\_manuscript.py} verifies that every
paper label is present and that
